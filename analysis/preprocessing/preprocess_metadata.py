...
def preprocess_metadata(args, embeddings_dict, metadata, meta_var, token_shape):    
    ...
    for run_id, emb in embeddings_dict.items():  # iterate over windowed embeddings
        ...
        if n_windows < args.n_windows_per_run_for_metadata:
            if token_shape[0] == -1:
                n_windows_to_sample = 1 # if per sequence embedding we have only one "window" per run
            else:
                print(f"Not enough windows ({n_windows}) for run_id {run_id} to sample.")
                continue
        else :
            if args.n_windows_per_run_for_metadata == -1:
                n_windows_to_sample = min_n_windows
            else:
        ...
        X.append(emb)  # keep windows unflattened (n_windows, emb_dim)
        y.extend([meta_value] * n_windows)
        groups.extend([animal_id] * n_windows)
        run_ids.extend([run_id] * n_windows)

    X = np.concatenate(X, axis=0)              # (n_total_windows, emb_dim)
    y = np.array(y)
